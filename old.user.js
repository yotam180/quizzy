...
window.msg_in_group = function(sender, origin, message) {
	...
	else if (is_answer_msg(message.body)) {
...
function bad_ans(name) {
	API.sendTextMessage(GROUP_ID, name + ", תשובתך אינה נכונה.");
};
...
window.answer_question = function(sender, answer) {
	...
	if (window.answerers.length > 0) {
		API.sentTextMessage(sender, "כבר ענו על שאלה זו!");
		return;
	}
